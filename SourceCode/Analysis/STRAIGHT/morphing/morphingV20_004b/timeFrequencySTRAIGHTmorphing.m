...
for ii=1:nAxis1
    ...
    ap2on2faxis = mObject2.aperiodicityIndex(:,iFloor)+dAp;
    ...
    switch mixMethod
        case 'linear'
            ...
            sgram2on2faxis = mObject2.spectrogram(:,iFloor)+dSgram;
        ...
        case 'log'
            ...
            sgram2on2faxis = log(mObject2.spectrogram(:,iFloor))+dSgram;
            sgram2on1faxis = sgram2on2faxis(fIndex);
    ...
    if (mObject2.F0(iFloor)>0) & (mObject2.F0(min(iFloor+1,nAxis2))>0)
        ...
        morphedF0(ii) = mixRate.F0*(log(mObject2.F0(iFloor))+dF0)+morphedF0(ii);
