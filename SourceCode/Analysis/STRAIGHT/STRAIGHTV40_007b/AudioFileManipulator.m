...
classdef AudioFileManipulator < handle
    ...
    methods
        ...
        function readProperties(object, inputFilePath)
            object.signal = audioread(inputFilePath);
            object.information = audioinfo(inputFilePath);
        end

        % 読み込んだ信号をその量子化ビット数に変換するメソッド
        function quantize(object)
            object.signal = object.signal .* (2 ^ (object.information.BitsPerSample - 1));
        end

        % 音声信号を保存するメソッド
        function saveWaveFile(object, outputFilePath, waveform)
            audiowrite(outputFilePath, waveform, object.information.SampleRate, 'BitsPerSample', object.information.BitsPerSample);
        end
