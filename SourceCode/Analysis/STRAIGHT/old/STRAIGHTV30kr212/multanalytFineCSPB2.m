function pm=multanalytFineCSPB2(x,fs,f0floor,nvc,nvo,mu,mlt);

%       Dual waveleta analysis using cardinal spline manipulation
%               pm=multanalytFineCSPB2(x,fs,f0floor,nvc,nvo,mu,mlt)
%       Input parameters 
%               
...
%       30/Oct./1996
%		03/Jan./2001 revision on mother wavelet
...
for ii=1:nvc
  ...
  wd1=exp(-pi*(t/t0/mu).^2).*exp(i*2*pi*mlt*t/t0); 
  wd2=max(0,1-abs(t/t0)).*exp(i*2*pi*mlt*t/t0);
  wd2=wd2(abs(wd2)>0);
  wwd=conv(wd2,wd1);
