...
% Last Modified by GUIDE v2.5 19-Oct-2009 15:44:38
...
wavwrite(outSignal,STRAIGHTobject.samplingFrequency,16,[path file]);
syncGUIStatus(handles)
...
if exist('writeSTFFile') == 2
    [file,path] = uiputfile({'*.mat','MAT files (*.mat)';'*.stf','STF files (*.stf)'},'Save the STRAIGHT object', outFileName);
else
...
if strcmpi(file(end-3:end), '.stf')
    writeSTFFile([path file], STRAIGHTobject);
else
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if exist('readSTFFile') == 2
    [file,path] = uigetfile({'*.mat', 'MAT files (*.mat)';'*.stf','STF files (*.stf)'},'Select STRAIGHT object or STF file to load');
else
...
if strcmpi(file(end-3:end), '.stf')
    originalSTRAIGHTobject = get(handles.TandemSTRAIGHThandler,'userdata');
    STRAIGHTobject = readSTFFile([path file], originalSTRAIGHTobject.waveform);
else
    %pathReg = regexprep(path,'\s','\\ ');
