...
if isfield(sourceStruture,'additionalInformation')
    f0 = min(sourceStruture.additionalInformation.controlParameters.f0ceil,sourceStruture.f0);
elseif isfield(sourceStruture,'controlParameters')
    f0 = min(sourceStruture.controlParameters.f0ceil,sourceStruture.f0);
else
...
for ii = 1:nFrames
    ...
    if ~isempty(fixedCenterFrequencies) % 21/Nov/2013 HK safeguard
        frequencyAxisForAperiodicity = [originalCenterFrequencies;fixedCenterFrequencies];
    else
