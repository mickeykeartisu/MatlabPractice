...
bit_ref = 24;
[compensate_db, y_max, y_max_fact] = defCa
librationParams(y_ref, fs_ref, refdb);
...
%             mkdir(outputDirName_2_norm)
            for threshold_dB_num = 1:length(threshold_dB)
                for v = 1:length(maxfreq),
                    outputDirName_2 = [outputDirName 'test_' num2str(dir_num) '/kneeWidth_dB_' num2str(kneeWidth_dB, '%02d') 'dB/' num2str(setGain(setGain_num)) 'dB/' num2str(maxfreq(v)) 'Hz/'];
                    outputDirName_2_norm = [outputDirName 'test_' num2str(dir_num) '/kneeWidth_dB_' num2str(kneeWidth_dB, '%02d') 'dB/normalization/' num2str(setGain(setGain_num)) 'dB/' num2str(maxfreq(v)) 'Hz/'];
                    mkdir(outputDirName_2)
                    mkdir(outputDirName_2_norm)
...
%                                 clear gainS;
        ...
        %                 outputName = [inputName '_' num2str(setGain) 'dB_' num2str(T(t)) 'band_hanning' num2str(maxfreq(v)) 'Hz_Obscurity']; %不明瞭化用
                        audiowrite([outputDirName_2_norm outputName '_normalization.wav'],ReX_norm, fs, 'BitsPerSample',nbit)
