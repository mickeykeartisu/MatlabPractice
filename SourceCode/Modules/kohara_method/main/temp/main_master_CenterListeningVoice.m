...
for dir_num = 4:dir_num_max,
% for dir_num = 4:4,
    inputDirName_2 = [inputDirName 'test_' num2str(dir_num) '/'];
    outputDirName_2 = [outputDirName 'test_' num2str(dir_num) '/'];
...
%                 outputName = [inputName '_' num2str(setGain) 'dB_' num2str(T(t)) 'band_hanning' num2str(maxfreq(v)) 'Hz_Obscurity']; %不明瞭化用
                audiowrite([outputDirName_2 outputName '.wav'],ReX, fs, 'BitsPerSample',nbit)

                ReX_width = [cut_wave length(ReX)-cut_wave];
                [ReX_norm] = getSig4SplNormalization_2(ReX, fs, compensate_db, setDB, ReX_width);

    %             ave = mean(ReX_norm)
    % 
    %             peak = max(ReX_norm)

    %             outputDirName = ['../voice_data/shinmitsu_test/normalization/' num2str(maxfreq(v)) 'Hz/'];
        %         outputName = [inputName '_' num2str(empha_Hz) 'Hz_' num2str(setGain) 'dB_' num2str(T(t)) 'band_taper_hanning' num2str(maxfreq) 'Hz'];
                outputName = [inputName '_' num2str(setGain) 'dB_' num2str(T(t)) 'band_hanning' num2str(maxfreq(v)) 'Hz'];
%                 outputName = [inputName '_' num2str(setGain) 'dB_' num2str(T(t)) 'band_hanning' num2str(maxfreq(v)) 'Hz_Obscurity']; %不明瞭化用
                audiowrite([outputDirName_2 outputName '_normalization.wav'],ReX_norm, fs, 'BitsPerSample',nbit)
%                 audiowrite([outputDirName_2 outputName '_normalization_max.wav'],ReX_norm/4, fs, 'BitsPerSample',nbit)
        ...
        [X_norm] = getSig4SplNormalization_2(X, fs, compensate_db, setDB, X_width);

        outputDirName2 = ['../voice_data/shinmitsu_test/normalization/original_normalization/' ];
        audiowrite([outputDirName_2 inputName '_normalization.wav'],X_norm, fs, 'BitsPerSample',nbit)
        audiowrite([outputDirName_2 inputName '_normalization_max.wav'],X_norm/4, fs, 'BitsPerSample',nbit)
    end
end
% close(hw)
disp('Completion');