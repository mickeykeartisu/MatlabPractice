...
phoneme_list = ["vowels", "consonants"];
...
for mask_index = 1 : length(mask_list)
    for file_index = 1 : 50
        ...
        for spectrogram_index = 1 : length(spectrogram_list)
            ...
            x = ((0:fftl/2)*spectrogram.sample_rate/fftl)';
            ltas_decibel = 10 * log10(spectrogram.long_term_average_spectrum);
            plot(x, ltas_decibel, plot_format_list(spectrogram_index), "DisplayName", spectrogram_list(spectrogram_index), "LineWidth", 3);
        end
        xlim([0 floor(spectrogram.sample_rate / 4)]);
        ylim([-60 10]);
        set(gca, "FontSize", font_size);
...
for spectrogram_index = 1 : length(spectrogram_list)
    for file_index = 1 : 50
        ...
        hold on;
        legend;
        title("Set1\_word " + int2str(file_index) + ", LTAS Comparison between noMask and withMask ", "FontSize", font_size);
        xlabel("Frequency [Hz]", "FontSize", font_size);
        ylabel("Magnitude [dB]", "FontSize", font_size);
        for mask_index = 1 : length(mask_list)
            %% load dynamic feature World mat file
...
for phoneme_index = 1 : length(phoneme_keys)
    ...
    ylabel("Magnitude [dB]", "FontSize", font_size);
    for mask_index = 1 : length(mask_list)
        long_term_average_spectrum_path = "D:/名城大学/研究室/研究/Sources/MatFiles/4モーラ単語リスト/Set1/" + mask_list(mask_index) + "/LongTermAverageSpectrum/WORLD/phoneme/phoneme_index " + int2str(phoneme_index) + ".mat";
        long_term_average_spectrum = load(long_term_average_spectrum_path);
        phoneme_long_term_average_spectrum = squeeze(long_term_average_spectrum.phoneme_long_term_average_spectrum);
...
vowel_long_term_average_spectrum = zeros(length(phoneme_list), length(mask_list), length(phoneme_long_term_average_spectrum));
for phoneme_list_index = 1 : length(phoneme_list)
    window = figure;
    window.WindowState = "maximized";
    grid on;
    hold on;
    ...
    for phoneme_index = 1 : length(phoneme_keys)
        for mask_index = 1 : length(mask_list)
            ...
            if sum(strcmp(phoneme_keys(phoneme_index), vowels_list)) == 1
                vowel_long_term_average_spectrum(1, mask_index, :) = vowel_long_term_average_spectrum(1, mask_index, :) + reshape(phoneme_long_term_average_spectrum, 1, 1, length(phoneme_long_term_average_spectrum));
            else
                vowel_long_term_average_spectrum(2, mask_index, :) = vowel_long_term_average_spectrum(2, mask_index, :) + reshape(phoneme_long_term_average_spectrum, 1, 1, length(phoneme_long_term_average_spectrum));
            end
        end
    end
    ...
    x = ((0:fftl/2)*long_term_average_spectrum.sample_rate/fftl)';
    for mask_index = 1 : length(mask_list)
        if phoneme_list(phoneme_list_index) == "vowels"
            ltas_decibel = 10 * log10(vowel_long_term_average_spectrum(1, mask_index, :) ./ length(vowels_list));
        else
            ltas_decibel = 10 * log10(vowel_long_term_average_spectrum(2, mask_index, :) ./ (length(phoneme_keys) - length(vowels_list)));
        end
        plot(x, squeeze(ltas_decibel), plot_format_list(mask_index), "DisplayName", mask_list(mask_index), "LineWidth", 3);
    end
    title("Set1 LTAS Comparison of " + phoneme_list(phoneme_list_index), "FontSize", font_size);
    xlim([0 floor(spectrogram.sample_rate / 4)]);
    ylim([-80 2]);
    set(gca, "FontSize", font_size);
    phoneme_png_path = "D:/名城大学/研究室/研究/Outputs/4モーラ単語リスト/Set1/both/LongTermAverageSpectrum/phoneme/" + phoneme_list(phoneme_list_index) + ".png";
    phoneme_emf_path = "D:/名城大学/研究室/研究/Outputs/4モーラ単語リスト/Set1/both/LongTermAverageSpectrum/phoneme/" + phoneme_list(phoneme_list_index) + ".emf";
    saveas(gcf, phoneme_png_path);
    saveas(gcf, phoneme_emf_path);
    delete(gcf);
end